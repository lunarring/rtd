...
class Posteffect:
    def __init__(
        self,
        device="cuda:0",
        motion_adaptive_blend=True,
        use_diffusion_field=True,
        enable_recent_motion_color_boost=False,
        enable_particle_effect=True,
        particle_image_path=get_repo_path("materials/images", __file__),
        enable_upward_offset=True,
    ) -> None:
        self.device = device
    ...
    def load_next_particle_image(self):
        ...
        image_files = [f for f in os.listdir(self.particle_image_path) if f.lower().endswith(valid_extensions)]
    ...
    def generate_smooth_random_field(self, height, width):
        ...
        smoothed_channels = [apply_gaussian_blur(random_field[..., i]) for i in range(2)]
    ...
    def warp_tensor(self, img, flow):
        ...
        grid_y, grid_x = torch.meshgrid(
            torch.linspace(-1, 1, H, device=self.device), torch.linspace(-1, 1, W, device=self.device), indexing="ij"
        )
    ...
    def process(
        self,
        img_diffusion,
        img_mask_segmentation,
        img_optical_flow,
        flow_gain,
        reverb_gain,
        background_image_gain,
        mod_button1,
        sound_volume_modulation,
    ):
        ...
        torch_img_mask_segmentation = lt.resize(
            torch_img_mask_segmentation, size=(torch_img_diffusion.shape[0], torch_img_diffusion.shape[1])
        )
        ...
        if self.enable_particle_effect and self.particle_image_path is not None and mod_button1:
            ...
            if self.accumulated_warped_particle is None:
                ...
                grayscale = 0.299 * output_to_render[..., 0] + 0.587 * output_to_render[..., 1] + 0.114 * output_to_render[..., 2]
